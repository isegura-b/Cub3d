...
{
    //hacia que lado tiene los bloques mas cercano
    if (var->ray_dir_x < 0)
    {
        var->step_x = -1;
        var->side_dist_x = (var->pos_x - var->map_x * WALL) * var->delta_dist_x / WALL;
    }
    else
    {
        var->step_x = 1;
        var->side_dist_x = ((var->map_x + 1) * WALL - var->pos_x) * var->delta_dist_x / WALL;
    }
    if (var->ray_dir_y < 0)
    {
        var->step_y = -1;
        var->side_dist_y = (var->pos_y - var->map_y * WALL) * var->delta_dist_y / WALL;
    }
    else
    {
        var->step_y = 1;
        var->side_dist_y = ((var->map_y + 1) * WALL - var->pos_y) * var->delta_dist_y / WALL;
    }
}
